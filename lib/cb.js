...
exports.initialize = function initializeDataSource(dataSource, callback) {
	...
	if(options.mappingPort) {
        ...
        net.createServer(function(from) {
            var to = net.createConnection({
                host: options.host,
                port: options.n1qlport
            });
            from.pipe(to);
            to.pipe(from);
        }).listen(8093, 'localhost');

        net.createServer(function(from) {
            var to = net.createConnection({
                host: options.host,
                port: options.memport
            });
            from.pipe(to);
            to.pipe(from);
        }).listen(11210, 'localhost');

        options.connectUrl = "http://localhost:8091";  //do not attach any port number such as 8901, will cause error code 24 which means "Error while establishing TCP connection"
        options.n1qlUrl = 'http://localhost:8093';
	} else {
        options.connectUrl = "http://" + options.host;  //do not attach any port number such as 8901, will cause error code 24 which means "Error while establishing TCP connection"
...
CouchbaseDB.prototype.connect = function (callback) {
  ...
  if (self.db) {
	...
	self.bucket = self.myCluster.openBucket(self.settings.bucket, self.settings.password,function (error) {
...
var cbquery = function (sql,selfThis, callback) {
    ...
    self.bucket.enableN1ql([self.settings.n1qlUrl]);
 ...
 */
CouchbaseDB.prototype.findById = CouchbaseDB.prototype.find = function find(model, id, callback) {
	var self = this;
...
CouchbaseDB.prototype.create = function (model, data, callback) {
    ...
    self.bucket.insert(createDataBuilderObj.getIdKey(),createDataBuilderObj.getPreparedData(),function(err, result) {
			if(err){
				callback(err,null);
				return;
			}
...
CouchbaseDB.prototype.updateOrCreate = function updateOrCreate(model, data, callback) {
	...
	// 		return;
	// 	}
		updateOneDoc(self,self.bucket,createDataBuilderObj.getIdKey(),model,createDataBuilderObj.getPreparedData(),callback);
	// });
};
...
var removeOneDoc = function(docid, thisRef,cb){
    ...
    self.bucket.remove(docid,function(err, result) {
			if(err){
				cb(err,null);
				return;
			}
...
CouchbaseDB.prototype.updateAttributes = function updateAttrs(model, id, data, callback) {
	...
	// 		return;
	// 	}
		updateOneDoc(self,self.bucket,idkey,model,data,callback);
	// });
};
